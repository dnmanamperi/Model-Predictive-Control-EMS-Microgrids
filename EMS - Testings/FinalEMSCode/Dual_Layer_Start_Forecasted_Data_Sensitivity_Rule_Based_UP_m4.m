...
for k=1:Nday
    ...
    for i=1:Nh
        ...
        f_mpc(idx+2)=-dt*Texp_base;  % export revenue pole wise
        f_mpc(idx+3)=c_deg*dt;             % Pch degradation
        f_mpc(idx+4)=c_deg*dt;             % Pdis degradation
        f_mpc(idx+5:idx+4+Npoles)=T_exp_vec*dt; % Curtailment pole wise , c_curt - scaler value, not matrix
    end
    ...
    for i=1:Nh
        ...
        ub(idx+1)=Pgrid_max;
        % Pexp
        ub(idx+2)=Pgrid_max/5; % pole pv e time step eke max eka
         %lb(idx+2:idx+1+Npoles)=max(0,0.5*PV(:,idx_start+i-1)-Load(:,idx_start+i-1)); % pole load e time step eke min eka (negative export)
        % Pch,Pdis
        ub(idx+3)=Pbat_max;
        ub(idx+4)=Pbat_max;
        % Curtailment
        ub(idx+5:idx+4+Npoles)=0.5*PV(:,idx_start+i-1);   % PV should be Npoles × time       % can limit curtailment from here - fairness constraint
    ...
    %     row2=zeros(1,nvar); row2(idx+2)=1;
    %     A=[A;row1;row2];
    ...
    % POLE EXPORTS
    Pexp(k)=x(1,2);        % store export for each pole
