...
for k=1:Nday
    ...
    for i=1:Nh
        ...
        ub(idx+1)=Pgrid_max;
        % Pexp
        ub(idx+2:idx+1+Npoles)=PV(:,idx_start+i-1); % pole pv e time step eke max eka
        ...
        % Curtailment
        ub(idx+4+Npoles:idx+3+2*Npoles)=1*PV(:,idx_start+i-1);   % PV should be Npoles × time       % can limit curtailment from here - fairness constraint
