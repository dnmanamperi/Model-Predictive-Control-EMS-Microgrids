...
% import TOU , export grid support , curtailment penalty LP
% rule base voltage part eka add kara
% pole wise curtailment + tariff add kara - wada na
...
a=0.05;
Vmax = 1+a;   % safety margin
Vmin = 1-a;   % safety margin
cmin = 12;   % min export tariff (LKR/kWh)
cmax = 18;   % max export tariff (LKR/kWh)
V0=1;         % reference voltage (p.u.)


% fairness weights (optional)
w = ones(Npoles,1);   % equal curtailment cost
u_min_store = zeros(Npoles,Nday);
...
% u_min_store = zeros(Npoles, Nday);
beta = zeros(Npoles,1);
Vpcc_1 = Vpcc(1:Nday);  % PCC voltage for the first 24h (96 time steps)
for k = 1:Nday
    ...
    A1 = -S_pu;
    b1 = Vmax - (Vpcc(k) + S_pu*((PV_now - Load_now)/3)/(S_base/1000));
    
    A2 = S_pu;
    b2 = (Vpcc(k) + S_pu*((PV_now - Load_now)/3)/(S_base/1000)) - Vmin;
    ...
    V_est = Vpcc(k) + S_pu*((PV_now - Load_now)/3)/(S_base/1000);  % estimated voltage without curtailment
    disp(V_est)
    size(A1)
    size(b1)
    size(A2)
    ...
    size(Aineq)
    size(bineq)
    if ((any(PV_now(:) > 0)) && (any(V_est(:) > Vmax)))
    
        [u_opt,~,exitflag] = linprog(f,Aineq,bineq,[],[],lb,ub,options);

        if exitflag ~= 1
            warning("Optimization did not converge @ k=%d",k);
            u_opt = zeros(Npoles,1);
        end
    ...
    Texp_pole_customer_day(:,k) = cmin + (cmax - cmin)*(1 - beta);

end

Texp_pole_customer = repmat(Texp_pole_customer_day,1,2);

% Texp_pole_customer = Texp_pole_customer_day;
...
for k=1:Nday
    ...
    %% Minimum Curtailment for Voltage Safety
    %% =============================

                                    % PV_now   = PV(:,k);
                                    % Load_now = Load(:,k);

                                    % % decision variable: u = curtailment at each pole
                                    % % f_upper = w;   % minimize weighted curtailment

                                


                                    % f = w;     % minimize w_j * u_j
                                    ...
                                    % % Upper voltage constraint
                                    % % V <= Vmax

                                    % A1 = -S;
                                    % b1 = Vmax - (V0 + S*(PV_now - Load_now));

                                    % % Lower voltage constraint
                                    % % V >= Vmin

                                    % A2 = S;
                                    % b2 = (V0 + S*(PV_now - Load_now)) - Vmin;
                                    ...
                                    % options = optimoptions('linprog','Display','none');
                                    % V_est = V0 + S*(PV_now - Load_now);



                                    % if (sum(PV_now) > 0 && max(V_est > Vmax))
                                    %     [u_opt,~,exitflag] = linprog(f,Aineq,bineq,[],[],lb,ub,options);

                                    %     if exitflag ~= 1
                                    %         warning("Optimization did not converge @ k= %d", k);
                                    %         u_opt = zeros(14,1);   % fallback
                                    %     end
                                    %     % disp(u_opt)
                                    %     % if isempty(u_opt)
                                    %     %     
                                    %     % end
                                    % else
                                    %     u_opt = zeros(14,1);   % no PV, no curtailment
                                    % end

                                    % u_min_store(:,k) = u_opt;  % store the minimum curtailment required for voltage safety in upper layer
                                    ...
                                    % %         elseif Vn >= 1.05
                                    % %             Texp_pole_customer_day(p,t) = 12;
                                                
                                    % %         else
                                    % %             Texp_pole_customer_day(p,t) = 18 - ((Vn - 0.95)/(1.05 - 0.95))*(18 - 12);
                                    % %         end
                                    ...
                                    % cmin = 12;   % min export tariff (LKR/kWh)
                                    % cmax = 18;   % max export tariff

                                    % beta = zeros(Npoles,1);   % initialize

                                    % for p = 1:Npoles
                                    %     if PV_now(p) > 0
                                    %         beta(p) = u_min_store(p,k) / PV_now(p);
                                    %         % disp(beta(p))
                                    %     else
                                    %         beta(p) = 0;  % or NaN if you want to mark "no PV available"
                                    %     end
                                    % end
                                    % % beta = u_min ./ PV_now;         % fraction of PV that must be curtailed
                                    % Texp_pole_customer_day(:,k) = cmin + (cmax - cmin)*(1 - beta);
                                    
                                    

                                    % Texp_pole_customer=repmat(Texp_pole_customer_day,1,2); % repeat for 48h, but we will only use the relevant part in each iteration
                                    % disp(Texp_pole_customer)
                                    % % T_curtail = T_exp_pole_customer; % dynamic curtailment penalty based on voltage for each pole and time step in the horizon
    ...
    for j = 1:Nh
        ...
        T_imp_j = Timp(k+j-1);
        f_mpc(jdx+1) = T_imp_j * dt;  % update import cost in cost vector for each time step based on TOU tariff
        T_curtail_j = Texp_pole_customer(:,k+j-1);
...
yyaxis right
plot(time_plot,Texp_pole_customer(1,1:96),'LineWidth',1.4)
...
%% =========================================================
%% MINIMUM CURTAILMENT REQUIRED (UPPER LAYER)
%% =========================================================

figure('Name','Upper Layer Required Curtailment','Position',[100 50 1200 900])

for p = 1:Npoles
    subplot(4,4,p)
    plot(time_plot,u_min_store(p,1:Nday),'LineWidth',1.3)
    xlabel('Hour')
    ylabel('kW')
    title(['Pole ' num2str(p)])
    grid on
...
sgtitle('Minimum Curtailment Required for Voltage Safety')
