...
data = pd.read_excel(
    r"BK28_general_purpose_load_solar_real_before_reconstructed.xlsx",sheet_name="Pole_Wise_Data_With_Total_Data"
)

timestamps = data.iloc[:,0]

P_import = data.iloc[:,1:2].values   # shape (T,14) in kW
P_export = data.iloc[:,2:3].values # shape (T,14) in kW
...
# Repeat or trim P_nat_kW to match T (length of Excel data)
if len(P_nat_kW) < T:
    reps = int(np.ceil(T / len(P_nat_kW)))
    P_nat_kW = np.tile(P_nat_kW, reps)[:T]
elif len(P_nat_kW) > T:
    P_nat_kW = P_nat_kW[:T]
...
for i in range(N):
    ...
    # Reconstruct load for all available data
    for t in range(T):
        if exp[t] == 0:
...
# Save
output.to_excel("Reconstructed_General_Purpose_Load_Solar_Real.xlsx", index=False)
...
print("Files saved:")
print(" - Reconstructed_Domestic_Load_Solar_Real.xlsx")
