...
r_per_km = 0.443
x_per_km = 0.08        # realistic LV cable reactance 0.0001
...
data = pd.read_excel(
    r"1 Fixed_Fixed_Fixed/MPC_Results_Fixed_Fixed_Fixed.xlsx",
...
pp.create_ext_grid(net, bus=bus_lv, vm_pu=1.0)
...
battery = pp.create_storage(
    ...
    bus=bus_lv,
    p_mw=0,
    max_e_mwh=0.08,   # optional energy capacity
...
for t in range(n_time):
    ...
    P_bat_results[t] = net.res_storage.p_mw.values[0]
...
df_out["Bat_Power_MW"] = P_bat_results

df_out.insert(0,"Timestamp",timestamps)

df_out.to_excel("1 Fixed_Fixed_Fixed/voltage_output_panda_powerflow_Fixed_Fixed_Fixed.xlsx", index=False)
