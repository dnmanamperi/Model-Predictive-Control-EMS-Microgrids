...
delta = 0.2;  
T_customer_export = 15;     
cmin = 15;             % min deviation from base export tariff for customers
cmax = 18;             % max deviation from base export tariff for customers
%% =========================================================
...
for k=1:Nday
    ...
    %% =========================================================
    %% PRECOMPUTE EXPORT TARIFF FOR ALL POLES AND TIMES - Optimization
    %% =========================================================
    ...
    %     f = -ones(Npoles,1);      % maximize export % min curtailment
    ...
    % Texp_pole_customer1 = repmat(Texp_pole_day,1,2);


    Texp_pole_day = zeros(Npoles,Nday);
    X_curtail     = zeros(Npoles,Nday);

    for t = 1:Nday
        
        PVt   = P_solar(t,1:Npoles)';     % PV available
        Loadt = P_load(t,1:Npoles)';      % Load
        V0    = Vpcc(t)*ones(Npoles,1);   % Base voltage
        
        Pnet = PVt - Loadt;   % Net injection without curtailment
        
        % Objective: minimize curtailment
        f = ones(Npoles,1);
        
        % Voltage constraints:
        % V = V0 + S*(Pnet - X_curtail)
        ...
        if exitflag ~= 1
            x = PVt;   % worst case: full curtailment
        end
        
        X_curtail(:,t) = x;
        
        % Tariff based on curtailment ratio
        % if min(PVt)==0 && max(PVt)==0
        %     tariff =0;
        % else
        %     tariff = cmax - (cmax - cmin).*(x ./ max(PVt));
        % end
        tariff = cmax - (cmax - cmin).*(x ./ max(PVt,1e-6));
        Texp_pole_day(:,t) = tariff;
...
filename = '5 TOU_Grid_Support_Optimization/MPC_Results_TOU_Grid_Support_Optimization.xlsx';
