...
data = readtable("Forecasted_14_Pole_Load_Solar.xlsx"); %forecatsed values for next 48h (96 time steps)
...
%% =========================================================
%% POLE-WISE LOAD & PV PROFILES - FORECASTED
%% =========================================================
...
sgtitle('Individual Pole Load and Solar Profiles (Forecasted)')

%% =========================================================
%% POLE-WISE LOAD & PV PROFILES - ACTUAL
%% =========================================================
...
for p = 1:Npoles
    subplot(4,4,p)
    plot(time, Load_real(p,:), 'b','LineWidth',1.2); hold on
    plot(time, PV_real(p,:),   'r','LineWidth',1.2);
    title(['Pole ' num2str(p)])
    xlabel('Time Step - 15 Minute Intervals'); ylabel('kW'); grid on
    if p==1
        legend('Load','PV')
...
sgtitle('Individual Pole Load and Solar Profiles (Actual)')
...
Pgrid_max_import = 64.9;  % grid limit (updated to match import limit) 
Pgrid_max_export = 97.8/2;  % export limit
c_deg  = 4.78;   % battery degradation
% c_curt = 22;   % curtailment penalty
...
delta = 0.2;  
T_customer_export = 15;                  % max deviation from base import
...
%% =========================================================
%% TOU EXPORT TARIFF (RULE-BASED)
%% =========================================================
Texp_day = zeros(1,Nday);
...
for k=1:Nday
    ...
    %% =========================================================
    %% PRECOMPUTE EXPORT TARIFF FOR ALL POLES AND TIMES - RBC
    %% =========================================================
    
    Texp_pole_customer_day = zeros(Npoles, Nday);
    ...
    for t = 1:Nday
        ...
        for p = 1:Npoles
            ...
            if Vn <= 0.95
                Texp_pole_customer_day(p,t) = 18;
                
            elseif Vn >= 1.05
                Texp_pole_customer_day(p,t) = 15;
                
            else
                Texp_pole_customer_day(p,t) = 18 - ((Vn - 0.95)/(1.05 - 0.95))*(18 - 15);
            end
    ...
    Texp_pole_customer1=repmat(Texp_pole_customer_day,1,2); % repeat for 48h, but we will only use the relevant part in each iteration
    ...
    PV_Forecasted_with_Real_State_Update = zeros(Npoles, 192);
    Load_Forecasted_with_Real_State_Update = zeros(Npoles, 192);
    
    PV_Forecasted_with_Real_State_Update(:,k) = PV_real(:,k);
    Load_Forecasted_with_Real_State_Update(:,k) = Load_real(:,k);
    
    PV_Forecasted_with_Real_State_Update(:,k+1:192) = PV(:,k+1 :192 );
    Load_Forecasted_with_Real_State_Update(:,k+1:192) = Load(:,k+1 :192);
    ...
    for j = 1:Nh
        ...
        T_exp_j = Texp(k+j-1);
        f_mpc(jdx+2) = -T_exp_j * dt;  % update export cost in cost vector for each time step based on TOU tariff
        
        T_curtail_j = Texp_pole_customer1(:,k+j-1);
        
        f_mpc(idx+5:idx+4+Npoles) = -dt*T_curtail_j;
    ...
    for i=1:Nh
        idx=(i-1)*nv_per;
        % Pimp max limit
        ub(idx+1)=Pgrid_max_import;
        % Pexp max limit
        ub(idx+2)=Pgrid_max_export; 
        % Pch,Pdis
        ub(idx+3)=Pbat_max;
        ub(idx+4)=Pbat_max;
        % Curtailment
        ub(idx+5:idx+4+Npoles)=0.3*PV_Forecasted_with_Real_State_Update(:,idx_start+i-1);   % PV should be Npoles × time       % can limit curtailment from here - fairness constraint
    ...
    for i=1:Nh
        ...
        beq_i = sum(Load_Forecasted_with_Real_State_Update(:,idx_start+i-1)) - sum(PV_Forecasted_with_Real_State_Update(:,idx_start+i-1));
    ...
    % TARIFF
    Texp_pole_customer(:,k)=Texp_pole_customer1(:,k);
...
plot(time_plot, Load_tot_real(1:Nday), 'k', time_plot, PV_tot_real(1:Nday), 'g','LineWidth',1.4)
...
for k = 1:Nday
    for p = 1:Npoles
        ...
        Pnet_pole(p,k) = Load_real(p,k) - (PV_real(p,k) - Curt_pole(p,k));     % Pole import --> (+)
    end
end
...
for k = 1:Nday
    for p = 1:Npoles
        Psche_soalr_pole(p,k) = PV_real(p,k) - Curt_pole(p,k);
    end
end
...
filename = '4 TOU_Grid_Support_RBC/MPC_Results_TOU_Grid_Support_RBC.xlsx';

% --- System level ---
SystemTable = table;

SystemTable.Time = time_excel(1:Nday);
SystemTable.Load_total  = Load_tot_real(1:Nday)';
SystemTable.Solar_total = PV_tot_real(1:Nday)';
...
for p = 1:Npoles
    PowerFlowTable.(['Pole_' num2str(p) '_Load_kW']) = Load_real(p,1:Nday)';
end
for p = 1:Npoles
...
%% =========================================================
figure('Name','Voltage vs Tariff (Pole 14)')

yyaxis left
plot(time_plot,V_poles(14,1:Nday),'LineWidth',1.4)
ylabel('Voltage (pu)')

yyaxis right
plot(time_plot,Texp_pole_customer(14,1:Nday),'LineWidth',1.4)
ylabel('Export Tariff (LKR/kWh)')

legend('Voltage (pu)','Export Tariff')
xlabel('Time Step - 15 Minute Intervals')
title('Voltage–Tariff Relationship (Pole 14)')
grid on


%% =========================================================
%% IMPORT TARIFF (Comparison)
%% =========================================================

t = 1:length(Timp);

figure;
plot(t, Timp, 'LineWidth', 2);
hold
plot(t, Texp, 'LineWidth', 2);
xlabel('Time Step (15 min)');
ylabel('Tariff');
title('Time-of-Use Import Tariff');
grid on;
...
%% POLE-WISE CURTAILMENT
%% =========================================================
% figure('Name','Pole-wise Curtailment')
for p = [10,13,14]
    % subplot(1,1,p)
    figure('Name','Pole-wise Curtailment')
    if exist('Curt_pole','var')
    ...
    set(gcf, 'Color', 'w');
    title(['Pole ' num2str(p),' Curtailment'])
    xlabel('Time Step - 15 Minute Intervals'); ylabel('kW'); grid on
    % sgtitle('Pole-wise Curtailment')
end
% sgtitle('Pole-wise Curtailment')
...
yyaxis left
plot(time_plot, Load_tot_real(1:Nday), 'k','LineWidth',1.2); hold on
plot(time_plot, PV_tot_real(1:Nday), 'g','LineWidth',1.2)
plot(time_plot, Pimp, 'b','LineWidth',1.2)
plot(time_plot, Pexp, 'r','LineWidth',1.2)
plot(time_plot, Pch, '--b','LineWidth',1.2)
plot(time_plot, Pdis, '--r','LineWidth',1.2)
plot(time_plot, Curt, 'c','LineWidth',1.2)
ylabel('Power (kW)')

yyaxis right
plot(time_plot, SOC_pct, 'm','LineWidth',1.5)
ylabel('SOC (%)')

grid on
xlabel('Time Step (15 min)')

legend('Load','PV','Import','Export','Charge','Discharge','Curtailment','SOC')

title('EMS Operation Overview')
...
% Extract data
Voltage = V_poles(14,1:Nday)';
Tariff  = Texp_pole_customer(14,1:Nday)';

% Create table
T = table(time_plot, Voltage, Tariff, ...
    'VariableNames', {'TimeStep','Voltage_pu','ExportTariff_LKRkWh'});

% Write to Excel
writetable(T, 'Voltage_Tariff_Pole14.xlsx');





%pole wise customer export revenue calculation
Pexport_pole = zeros(Npoles,Nday);

for k = 1:Nday
    for p = 1:Npoles

        % Net surplus solar available for export
        Pexport_pole(p,k) = max(0,...
            PV_real(p,k) - Curt_pole(p,k) - Load_real(p,k));

    end
end

ExportRevenue_pole = zeros(Npoles,Nday);

for k = 1:Nday
    for p = 1:Npoles

        ExportRevenue_pole(p,k) = ...
            Pexport_pole(p,k) * ...
            Texp_pole_customer(p,k) * ...
            dt;

    end
end

TotalRevenuePerPole = sum(ExportRevenue_pole,2);
TotalCommunityRevenue = sum(TotalRevenuePerPole);

fprintf('\nTotal Export Payment = %.2f LKR\n', ...
    TotalCommunityRevenue);

RevenueTable = table((1:Npoles)', ...
    TotalRevenuePerPole,...
    'VariableNames',{'Pole','Export_Revenue_LKR'});

writetable(RevenueTable,filename,...
    'Sheet','Pole_Export_Revenue');